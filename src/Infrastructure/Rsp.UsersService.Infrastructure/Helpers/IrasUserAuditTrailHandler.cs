...
public class IrasUserAuditTrailHandler : IAuditTrailHandler
{
    ...
    public async Task<IEnumerable<UserAuditTrail>> GenerateAuditTrails(EntityEntry entry, IrasUser systemAdmin, IrasIdentityDbContext? context)
    {
        if (entry.Entity is not IrasUser irasUser)
        {
            return [];
        }
        ...
        switch (entry.State)
        {
            ...
            case EntityState.Modified:
                ...
                var modifiedAuditableProps = entry.Properties.Where
                (
                    p =>
                ...
                foreach (var property in modifiedAuditableProps)
                {
                    ...
                    else
                    {
                        updateAuditTrail.Description =
                            $"{property.Metadata.Name} was changed from {property.OriginalValue} to {property.CurrentValue}";
                    }
