...
public static class RemoveUserClaimsEndpoint
{
    public static async Task<Results<NotFound<string>, ValidationProblem, NoContent>> RemoveUserClaims<TUser>
    (
    ...
    ) where TUser : IrasUser, new()
    {
        ...
        var result = await userManager.RemoveClaimsAsync(user, claimsRequest.Claims.ConvertAll(c => new Claim(c.Key, c.Value)));
